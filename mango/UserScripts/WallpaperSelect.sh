...
wallpaper_current="$HOME/.config/mango/wallpaper_effects/.wallpaper_current"
...
focused_monitor=$(wlr-randr | grep '(preferred, current)' -B 10 | awk '/^[a-zA-Z0-9]/ { print $1; exit }')
...
# Monitor details
scale_factor=$(wlr-randr | awk -v mon="$focused_monitor" '$1 == mon { found=1 } found && /Scale:/ { print $2; exit }')
monitor_height=$(wlr-randr | awk -v mon="$focused_monitor" '$1 == mon { found=1 } found && /\(preferred, current\)/ { split($1, res, "x"); print res[2]; exit }')
...
apply_image_wallpaper() {
  ...
  # sleep 1
  # Update autostart.sh swaybg wallpaper path
  AUTOSTART="$HOME/.config/mango/autostart.sh"
  if grep -q "swaybg -m fill -i" "$AUTOSTART"; then
    # sed -i 's|swaybg -i [^ ]*|swaybg -i '"$image_path"'|' "$AUTOSTART"
    sed -i 's|^swaybg -m fill -i .*|swaybg -m fill -i "'"$image_path"'" \&|' "$AUTOSTART"
  fi
}
...
apply_video_wallpaper() {
  ...
  mpvpaper '*' -o "hwdec=auto load-scripts=no no-audio --loop --panscan=1" "$video_path" &
}
